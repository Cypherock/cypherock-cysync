...
export interface Trc20ListItem {
  id: string;
  symbol: string;
  name: string;
  market_cap?: number;
...
const createContractMapFromExistingList = (list: Trc20ListItem[]) => {
  const map: Record<string, Trc20ListItem | undefined> = {};
  ...
  for (const coin of list) {
    ...
    for (const platform in coin.platforms) {
      ...
      if (!tronCoinList[platform]) continue;
...
const getChangedContracts = (params: {
  ...
  coingeckoCoinMap: Record<string, Trc20ListItem | undefined>;
  existingCoinMap: Record<string, Trc20ListItem | undefined>;
}) => {
  const { sameContracts, coingeckoCoinMap, existingCoinMap } = params;
  const changedContracts: string[] = [];
  const changedCoinList: Trc20ListItem[] = [];
  const idChanges: {
    oldId: string;
    oldVersion?: string;
    newId: string;
...
const getRemovedContracts = (params: {
  ...
  const removedCoinList: Trc20ListItem[] = [];
...
const verifyDuplicateIds = (list: Trc20ListItem[]) => {
  const idSet = new Set();
...
const getAddedContracts = (params: {
  ...
  const addedCoinList: Trc20ListItem[] = [];
...
export const getTRC20TokenDifference = async (dontSaveToFile?: boolean) => {
