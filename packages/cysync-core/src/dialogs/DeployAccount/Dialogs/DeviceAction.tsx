...
import { IEvmCoinInfo, coinList } from '@cypherock/coins';
import {
  LangDisplay,
  DialogBox,
  DialogBoxBody,
...
export const DeviceAction: React.FC = () => {
  ...
  const actionsList = React.useMemo<LeanBoxProps[]>(() => {
    const actions: LeanBoxProps[] = [
      {
        id: '1',
        text: lang.strings.deployAccount.x1Vault.actions.verifyCoin,
      ...
      },
      {
        id: '4',
        leftImage: rightArrowIcon,
        text: lang.strings.deployAccount.x1Vault.actions.verifyDetails,
        rightImage: getDeviceEventIcon(
          SignTransactionDeviceEvent.CONFIRMED,
  ...
  const displayText = lang.strings.deployAccount.x1Vault;

  const getMessageBoxes = () => {
    const messages: { text: string; variables?: any }[] = [];
    if (selectedAccount?.type === AccountTypeMap.subAccount) {
      messages.push({
        text: lang.strings.deployAccount.x1Vault.token.info,
        variables: {
          tokenName: assetName,
