...
async function initContracts() {
  ...
  if (!contracts['eth']) {
    contracts['eth'] = new (getStarknetApiJs()).Contract((await provider.getClassAt(ethContractAddress)).abi, ethContractAddress, provider);
  }

  if (!contracts['strk']) {
    contracts['strk'] = new (getStarknetApiJs()).Contract((await provider.getClassAt(strkContractAddress)).abi, strkContractAddress, provider);
  }
}
...
export const getBalance = async (
  ...
  await initContracts();
  const ethBalance = await (contracts.eth!).balanceOf(address) as bigint;
  if (assetId === 'strk') {
    const strkBalance = await (contracts.strk!).balanceOf(address) as bigint;
  }
