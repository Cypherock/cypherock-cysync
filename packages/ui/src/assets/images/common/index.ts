import addWalletIcon from './add-wallet.png';
import cysyncLogoBig from './logo-big.png';
import cysyncLogoSmall from './logo-small.png';
import closeIcon from './close.png';
import importWalletIcon from './import-wallet.png';
import recoverWalletIcon from './recover-wallet.png';
import joyStickProgressIcon from './joystickProgress.png';
import emailAuthProgressIcon from './emailAuthProgress.png';
import deviceAuthAsideImage from './deviceAuth.png';
import asideIcon from './aside.svg';
import successIcon from './success.svg';
import termsProgressIcon from './terms-progress.png';
import backIcon from './back.svg';
import cardProgressIcon from './card-progress.png';
import deviceAuthProgressIcon from './deviceAuthProgressbar.png';
import emailIcon from './email-icon.svg';
import blockSelectedIcon from './Block.svg';
import blockIcon from './Block-unselectedsvg.svg';
import disconnectedIcon from './disconnected.svg';
import failIcon from './fail.svg';
import joystickArrowCompletedIcon from './JoystickArrowCompleted.svg';
import joystickArrowIcon from './JoystickArrowDisabled.svg';
import joystickArrowSelectedIcon from './JoystickArrowSelected.svg';
import joystickIcon from './Joystick.png';
import cardTapAsideImage from './CardTapAsideImage.png';
import LogoOutlinedAsideImage from './LogoOutlineAside.png';
import joystickTrainingAsideImage from './JoystickTrainingAside.png';

export {
  addWalletIcon,
  asideIcon,
  backIcon,
  closeIcon,
  cysyncLogoBig,
  cysyncLogoSmall,
  importWalletIcon,
  recoverWalletIcon,
  termsProgressIcon,
  cardProgressIcon,
  emailAuthProgressIcon,
  joyStickProgressIcon,
  deviceAuthAsideImage,
  emailIcon,
  deviceAuthProgressIcon,
  blockIcon,
  blockSelectedIcon,
  disconnectedIcon,
  successIcon,
  failIcon,
  joystickArrowCompletedIcon,
  joystickArrowIcon,
  joystickArrowSelectedIcon,
  joystickIcon,
  cardTapAsideImage,
  LogoOutlinedAsideImage,
  joystickTrainingAsideImage,
};
